...
\section{Problem 1}
    ...
    \begin{proof}
        ...
        Therefore, since $\log\log 2 < 0$, $n\log(2) + \log \log (2) < n$ and by extension $(n\log(2) + \log \log (2))^p < n$.
        Therefore, $\frac{1}{(n\log(2) + \log \log (2))^p} > \frac{1}{n}$.
        If we sum this for $n \geq 3$, we find that $\sum_{n=3}^{\infty} \frac{1}{(n\log(2) + \log \log (2))^p} > \sum_{n=3}^{\infty} \frac{1}{n}$.
        Since $\sum_{n=1}^{\infty} \frac{1}{n}$ diverges to $\infty$, we can equally say that $\sum_{n=3}^{\infty} \frac{1}{n}$ diverges to $\infty$.
        By 

        However, since $n$ is monotonically increasing and unbounded, for every $m \in \mathbb{N}$, there is an $N_1$ such that if $n > N_1$, $n\log(2) + \log \log (2) \geq m$.

        \begin{gather}
            \frac{1}{n^{p - 1}} > \frac{1}{(n\log 2 + \log \log 2)^p}\\
            (n\log 2 + \log \log 2)^p > n^{p - 1}\\
            n \log(2) + \log \log 2 > n \text{ if } n \geq 3\\
            (n \log(2) + \log \log 2)^p > n^p > n^{p - 1} \text{ if } n \geq 3
        \end{gather}
    \end{proof}
