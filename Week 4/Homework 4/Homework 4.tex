...
\section{Various Theorems}
    ...
    \subsection{Decreasing Value is Greater than Alternating Partial Sums}
        If $a_k \geq 0$ for all $k \in \mathbb{N}$ and $a_k$ is decreasing, then for all $m,N \in \mathbb{N}$ where $m > N$, $\abs{\sum_{k = N}^{m} (-1)^k a_k} \leq a_N$.
        \begin{proof}
            Let $a_k \geq 0$ and $a_k \geq a_{k+1}$ for all $k \in \mathbb{N}$.
            Let $m,N \in \mathbb{N}$ where $m > N$.
            At this point, there are two cases: $m - N$ is odd or $m - N$ is even.

            Consider first the case when $m - N$ is odd.
            Without loss of generality, suppose that $k$ is even, so $(-1)^k = 1$.
            Since $a_k$ is decreasing, for any $j \in \mathbb{N}$, $a_j \geq a_{j+1}$.
            Therefore, for any $j \in \mathbb{N}$ where $N < 2j < 2j+1 \leq m$, $a_{2j} \geq a_{2j+1}$.
            This means that $a_{2j} - a_{2j+1} \geq 0$ and $0 \geq a_{2j+1} - a_{2j}$.
        \end{proof}
...
\section{Problem 1}
    ...
    \subsection{Solution (b)}
        ...
        Now the case that $p \leq 0$.
        \begin{proof}
            If $p \leq 0$, then for $q = -p$ (so $q \geq 0$), $\frac{1}{n^p} = n^q$.
            Since $n \in \mathbb{N}$ and $q \geq 0$, $n^q \geq 1$, so the sequence does not converge to $0$.
            Therefore, since a series converges only if its sequence converges to zero and $\frac{1}{n^p}$ does not converge to $0$, $\sum_{n = 1}^{\infty} \frac{1}{n^p}$ diverges if $p \leq 0$.
        \end{proof}

        Last, we consider the case where $0 < p \leq 1$.
        \begin{proof}
            Since $p > 0$ and $n + 1 > n$, $(n + 1)^p > n^p$ and by extension $\frac{1}{(n + 1)^p} < \frac{1}{n^p}$.
            Since the only potentially negative term in $\frac{(-1)^{n - 1}}{n^p}$ is $(-1)^{n-1}$, and that is negative if and only if $n$ is even, $\frac{(-1)^{n - 1}}{n^p}$ is negative if and only if $n$ is even.
            We can separate the terms added up in our series into the positive and negative terms.
        \end{proof}
\pagebreak
\section{Problem 2}
