...
\section{Problem 4}
    ...
    \pagebreak
    \subsection{Solution (a)}
        Without loss of generality, suppose that $\frac{p}{q} = \frac{r}{s}$ and $\frac{p}{q}$ is in lowest terms but $\frac{r}{s}$ is not.
        Take $\sqrt[q]{a^p}$.
        We can rewrite it as $\sqrt[q]{a^p} = a^{\frac{p}{q}}$.
        Since $\frac{r}{s}$ is not in lowest terms and $\frac{p}{q} = \frac{r}{s}$, there is some nonzero integer $t$ such that $\frac{r}{s} = \frac{p \cdot t}{q \cdot t} = \frac{p \cdot \frac{t}{t}}{q}$.
    \pagebreak
    \subsection{Solution (b)}
    \pagebreak
    \subsection{Solution (c)}
