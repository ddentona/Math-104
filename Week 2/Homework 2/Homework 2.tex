...
\section{Problem 5}
    ...
    \subsection{Solution (c)}
        ...
        \begin{proof}
            We proved in part (b) that since $K$ is compact, every open cover of $K$ consisting of open balls has a finite subcover.
            Suppose we take one of those finite subcovers of a cover consisting of open balls and call it $\{F_\alpha\}_{\alpha \in A}$.
            Since it is a cover, for every $y \in K$, $y \in F_\alpha$ for some $\alpha$.

            Every open ball $B_\varepsilon(x)$ is defined as $B_\varepsilon(x) = \left\{ y \in K \Big| d(y,x) < \varepsilon \right\}$.
        \end{proof}
\end{document}